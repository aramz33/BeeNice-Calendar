...
`cancelled` (schedule change) also spawns one. `completed` and `not_qualified` are
terminal — no task.
...
## Confirmed (Julien, 2026-06-24 / démo 2026-06-25)

`Refus` means the prospect was not available at that slot (not a hard no), so it **does**
spawn a reposition task. `MVN` (wrong number) was retired from booking dispositions (démo
25-06): it's a *lead* status, not a booking outcome — legacy `mvn` rows fold into
`not_qualified`.
