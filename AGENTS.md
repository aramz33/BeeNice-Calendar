...
Guidance for Codex and other AI agents when working in this repository.
...
Obsidian vault (single source of truth). At the beginning of each session, use the
`adam-vault` skill to load the project context from Obsidian before coding or planning.
...
- **Depth** -> `6 - Main Notes/Pro/BeeNice/LOG.md` (history + rationale),
  `6 - Main Notes/Pro/BeeNice/ARCHITECTURE.md` (technical map), `wiki/`
  (routing-design, microsoft-enterprise-auth, nylas-microsoft-oauth, etc.).
- **Discovery** -> notes tagged `agents`:
...
- **Open work / issues**: no separate issue folder — checklist items live in
  `TODO.md`; design notes for open questions live in `Pro/BeeNice/wiki/`. Triage
  vocab: see `docs/agents/triage-labels.md`.
...
- **After delivery or major decisions**: sync the outcome back to Obsidian via the
  project handoff flow so future agents resume from `TODO.md`/`LOG.md`; update
  `ARCHITECTURE.md` too when modules, routes, schema, providers, runtime topology, or
  major data flows changed.
